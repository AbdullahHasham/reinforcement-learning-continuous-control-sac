...
def train_custom(cfg_path=None):
    """
    Train model with custom reward function.
    Runs multiple trials for robustness.
    """
    if cfg_path is None:
        cfg_path = os.path.join(BASE_DIR, "config", "config_custom.yaml")
    cfg = load_config(cfg_path)

    # Create directories if they don't exist
    results_dir = os.path.join(BASE_DIR, "results", "custom")
    ...
    for trial in range(1, cfg['num_trials'] + 1):
        print(f"\n{'='*50}")
        print(f"Starting Custom Reward Trial {trial}/{cfg['num_trials']}")
        print(f"{'='*50}\n")

        # Create base environment
        env = gym.make(cfg['environment'])

        # Wrap with custom reward
        env = CustomRewardWrapper(env, cfg.get('reward_params', {}))

        # Create log directory for this trial
        log_dir = os.path.join(BASE_DIR, "logs", "custom", f"{cfg['algorithm']}_trial{trial}")
        ...
        cb = CheckpointCallback(
            ...
            name_prefix=f"{cfg['algorithm']}_custom"
        )
